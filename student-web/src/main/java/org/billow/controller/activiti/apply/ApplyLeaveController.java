package org.billow.controller.activiti.apply;
...
public class ApplyLeaveController {
	...
	public ModelAndView editLeave(LeaveDto leave) {
		...
		if (leave.getId() != null) {
			...
			if (leave != null && "7".equals(leave.getStatus())) {// 被驳回的
				viewName = PagePathCst.BASEPATH_ACTIVITI_APPLY + "leaveApplyRe";
				av.addObject("leaveDto", leave);
			}
		}
	...
	public ModelAndView findLeaveList(HttpSession session, LeaveDto leave) {
		...
		av.setViewName(PagePathCst.BASEPATH_ACTIVITI_APPLY + "leaveApplyList");
		return av;
	}
